...
export default function IncidentTimeline({ items = [] }: IncidentTimelineProps) {
  if (!items.length) {
    return (
      <div className="p-6 rounded-xl border border-border/40 bg-background/30 text-sm text-muted-foreground font-mono">
      ...
      {items.map((it) => (
        <div key={it.id} className="relative pl-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-xs font-mono uppercase tracking-wide text-muted-foreground flex items-center gap-1">
              <Calendar className="h-3.5 w-3.5" /> {it.date}
            </span>
            <span className="text-sm font-mono font-semibold">{it.title}</span>
            {it.resolved && (
              <span className="text-xs font-mono text-data-positive flex items-center gap-1">
                <Clock className="h-3.5 w-3.5" /> Resolved
              </span>
            )}
          </div>
          {it.description && (
            <p className="mt-1 text-xs font-mono text-accent-foreground/80 flex items-start gap-2">
