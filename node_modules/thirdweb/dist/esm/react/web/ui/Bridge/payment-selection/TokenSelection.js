...
export function TokenSelection({ paymentMethods, paymentMethodsLoading, client, onPaymentMethodSelected, onBack, destinationToken, destinationAmount, feePayer, }) {
    const theme = useCustomTheme();
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Text, { color: "primaryText", size: "md", children: "Select payment token" }), _jsx(Spacer, { y: "sm" }), _jsx(Container, { flex: "column", gap: "sm", style: {
                    ...
                    .map((method) => (_jsx(PaymentMethodTokenRow, { client: client, destinationAmount: destinationAmount, destinationToken: destinationToken, feePayer: feePayer, onPaymentMethodSelected: onPaymentMethodSelected, paymentMethod: method }, `${method.originToken.address}-${method.originToken.chainId}`))) })] }));
}
